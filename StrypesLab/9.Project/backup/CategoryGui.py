...
class CollectionGUI(tk.Tk):
    ...
    def create_widgets(self):
        ...
        self.result_listbox = tk.Listbox(self, width=80, height=20)
        self.result_listbox.grid(row=1, column=0, columnspan=4, padx=10, pady=10)
    ...
    def update_listbox(self):
        ...
        for item in self.collection_manager.data[category]:
            item_str = f"\"{item['title']}\"    ({item['author']} - {item['year']})"
            self.result_listbox.insert(tk.END, item_str)
    ...
    def edit_item(self):
        ...
        index = selection[0]
        category = self.category_var.get()
        item = self.collection_manager.read_item(category, index)

        self.title_entry.delete(0, tk.END)
        self.title_entry.insert(tk.END, item['title'])

        self.author_entry.delete(0, tk.END)
        self.author_entry.insert(tk.END, item['author'])

        self.year_entry.delete(0, tk.END)
        self.year_entry.insert(tk.END, item['year'])
