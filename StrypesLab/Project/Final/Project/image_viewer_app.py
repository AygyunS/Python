...
class ImageViewerApp(tk.Tk):
    def __init__(self):
        ...
        self.bind_all('<Control-Z>', lambda event: self.redo())

        # Shortcut (Ctrl+N)
        self.bind_all('<Control-n>', lambda event: self.open_image())

        # Shortcut (Ctrl+S)
        self.bind_all('<Control-s>', lambda event: self.save_image())

        # Shortcut (Ctrl+BackSpace)
        self.bind_all('<Control-BackSpace>', lambda event: self.delete_image())

        # Shortcut (Ctrl+R)
        self.bind_all('<Control-r>', lambda event: self.reset_image())
    ...
    def show_image_histogram(self):
        ...
        # Create a FigureCanvasTkAgg object to draw the histogram on
        fig, ax = plt.subplots()
        ax.hist(pixel_values, bins=256, range=(0, 256), density=True)
        ...
        # Buton za zatvarqne
        close_button = tk.Button(histogram_window, text="Изход", command=histogram_window.destroy)
    ...
    def red_eye_reduction(self, image):
        ...
        for (x, y, w, h) in eyes:
            ...
            red_mean = np.mean(r)
            # Create a mask based on the red channel intensity
            red_mask = cv2.inRange(r, int(red_mean * 1.1), 255)
            # Replace the red channel with the green channel in the masked region
            r[red_mask == 255] = g[red_mask == 255]
            eye = cv2.merge((b, g, r))
